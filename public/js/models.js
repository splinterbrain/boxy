    ...
    BOXY.BoxEditorView = Backbone.View.extend({ //The edit moodle
        ...
        'render':function () { //Edit moodle displays properties of whichever box is its target.
            ...
            $.getJSON("/icons", $.proxy(function (glyphs) {
                //Append the empty icon                                                                                                     
                this.$el.find("#iconScroll").append("<div data-icon='' class='icon selectedIcon'></div>");
...
//				BOXY.aCollection.remove(this.model);
        ...
        'onChange':function (e) {//Confers changes made in the editor to the model. 
            ...
            if (this.$el.find(".selectedIcon").length > 0) {
                this.model.set({
            ...
            for (var i = 0; i < BOXY.aCollection.models.length; i++) {
                ...
                if (model.isDirty) {
                    ...
                    model.save(null, {success:function (model) {
                        model.isDirty = false;
                        if (!_.any(BOXY.aCollection.models, function (m) {
                            return m.isDirty;
                        })) $("#saveIndicator").removeClass("saving").removeClass("error");
