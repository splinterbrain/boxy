...
Manager = {
    start:function (fontpath) {
        ...
        //Copy font file into cache
        exec("cp " + fontpath + " " + this.cachedir, function(err, stdout, stderr){
            if(err){
...
//        console.log(Manager.fontpath);     
        ...
        redisClient.smembers("glyph:add", function (err, glyphs) {
            ...
            exec("python " + Manager.buildscript + " add " + glyphs.join(" "), {cwd : Manager.cachedir}, function(err, stdout, stderr){
                if(err){
                    console.log("Error adding glyphs");
                    console.log(stderr);
                    return;
                }
                glyphs.unshift("glyph:add");
                redisClient.srem(glyphs);
                
                //Copy font back to web directory
                exec("cp " + path.join(this.cachedir, "glyphs.ttf") + " " + Manager.fontpath, function(err, stdout, stdin){
                    if(err){
                        console.log("Error copying file font file");
                        console.log(stderr);
                        throw new Error();
