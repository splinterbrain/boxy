...
Manager = {
    start:function (fontpath) {
        ...
        exec("cp " + fontpath + " " + path.join(this.cachedir, "glyphs.tff"), function(err, stdout, stderr){
            if(err){
...
//        console.log(Manager.fontpath);     
        ...
        redisClient.smembers("glyph:add", function (err, glyphs) {
            ...
            exec(pythonCmd, {cwd : Manager.cachedir}, function(err, stdout, stderr){
                ...
                exec("cp " + path.join(this.cachedir, "glyphs.ttf") + " " + Manager.fontpath, function(err, stdout, stderr){
                    if(err){
                        console.log("Error copying file font file");
                        console.log(stderr);
                        throw new Error();
